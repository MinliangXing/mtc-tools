...
package com.mtc.tools.schedule.dangdang.core.event.rdb;

import com.google.common.base.Strings;

import com.mtc.tools.schedule.dangdang.core.context.ExecutionType;
...
final class JobEventRdbStorage {
    ...
    private String getTableName(Connection conn, String tableName) throws SQLException {
        if (DatabaseType.valueFrom(conn.getMetaData().getDatabaseProductName()) == DatabaseType.PostgreSQL) {
    ...
    private void createJobExecutionTableAndIndexIfNeeded(final Connection conn) throws SQLException {
        ...
        try (ResultSet resultSet = dbMetaData.getTables(null, null, getTableName(conn, TABLE_JOB_EXECUTION_LOG), new String[]{"TABLE"})) {
            if (!resultSet.next()) {
    ...
    private void createJobStatusTraceTableAndIndexIfNeeded(final Connection conn) throws SQLException {
        DatabaseMetaData dbMetaData = conn.getMetaData();
        try (ResultSet resultSet = dbMetaData.getTables(null, null, getTableName(conn, TABLE_JOB_STATUS_TRACE_LOG), new String[]{"TABLE"})) {
            if (!resultSet.next()) {
    ...
    private void createTaskIdIndexIfNeeded(final Connection conn, final String tableName, final String indexName) throws SQLException {
        DatabaseMetaData dbMetaData = conn.getMetaData();
        try (ResultSet resultSet = dbMetaData.getIndexInfo(null, null, getTableName(conn, tableName), false, false)) {
            boolean hasTaskIdIndex = false;
            while (resultSet.next()) {
                if (getTableName(conn, indexName).equals(resultSet.getString("INDEX_NAME"))) {
                    hasTaskIdIndex = true;
                }
            }
            if (!hasTaskIdIndex) {
    ...
    private void createJobExecutionTable(final Connection conn) throws SQLException {
        ...
        String dbSchema = "CREATE TABLE `" + TABLE_JOB_EXECUTION_LOG + "` ("
                ...
                + (DatabaseType.valueFrom(conn.getMetaData().getDatabaseProductName()) == DatabaseType.PostgreSQL ?
                "`is_success` BOOLEAN NOT NULL, " : "`is_success` INT NOT NULL, ")
                + "`start_time` TIMESTAMP NULL, "
                + "`complete_time` TIMESTAMP NULL, "
                + "PRIMARY KEY (`id`));";
        try (PreparedStatement preparedStatement = conn.prepareStatement(getSql(conn, dbSchema))) {
    ...
    private void createTaskIdAndStateIndex(final Connection conn, final String tableName) throws SQLException {
        String sql = "CREATE INDEX " + TASK_ID_STATE_INDEX + " ON " + getTableName(conn, tableName) + " (`task_id`, `state`);";
        try (PreparedStatement preparedStatement = conn.prepareStatement(getSql(conn, sql))) {
            preparedStatement.execute();
        }
    }
    ...
    private String getSql(Connection conn, String sql) throws SQLException {
        if (DatabaseType.valueFrom(conn.getMetaData().getDatabaseProductName()) == DatabaseType.PostgreSQL) {
            return sql.replace("`", "");
        }
        return sql;
    ...
    private boolean insertJobExecutionEvent(final JobExecutionEvent jobExecutionEvent) {
        ...
        try (
                ...
                PreparedStatement preparedStatement = conn.prepareStatement(getSql(conn, sql))) {
